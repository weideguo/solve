...
def check_host_conn(host_info, redis_config_list, *args, **kargs):
    ...
    redis_send_client.expire(
        config.prefix_heart_beat + tag, config.host_check_success_time
    )
...
class SaltConn(AbstractConn, AbstractHost):
    ...
    def __init__(
        self, host_info, redis_config_list, redis_connect=None, *args, **kwargs
    ):
        self.salt = MySalt(c_path="/etc/salt/master")
        ...
        for ip in r.scan_iter():
            if r[ip]:
                ...
                self.redis_send_client.expire(
                    config.prefix_heart_beat + ip, config.host_check_success_time
                )
