...
class RemoteHost(MySSH, AbstractHost, AbstractThread):
    ...
    def __init__(
        ...
        host_info,
        redis_config_list,
        t_number=config.max_concurrent_thread,
        redis_connect=None,
    ):
        ...
        self.ip_tag = (
            host_info.get("uuid") or self._ip_tag
        )  # 使用uuid可以更精确控制单个主机 当一个ip存在多个连接时
        ...
        if redis_connect:
            ...
            rc = RedisConn(
                max_connections=config.remote_host_redis_pool_size
            )  # 每个远程连接使用自己的连接池  有些连接不能被共享  如进行blpop操作时/消息订阅时 因而在此需要大于2
