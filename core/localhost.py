...
class LocalHost(AbstractHost, AbstractThread):
    ...
    def __init__(
        ...
        redis_config_list,
        listen_tag,
        t_number=config.max_localhost_thread,
        redis_connect=None,
    ):
        ...
        if redis_connect:
            ...
            self.redis_connect = RedisConn(
                max_connections=config.localhost_redis_pool_size
            )  # 每个远程连接使用自己的连接池
        ...
        self.init_parallel(
            t_number, stop_able=False, join=True
        )  # 即使线程运行错误 也继续运行

    def redis_refresh(self):
        self.redis_send_client = self.redis_connect.refresh(
            self.redis_send_config, force=True
        )
        self.redis_log_client = self.redis_connect.refresh(
            self.redis_log_config, force=True
        )
    ...
    def real_func(self, cmd, cmd_uuid, ip_tag, *args, **kwargs):
        if cmd:
            # 本地执行只实现部分扩展命令
            self.single_run(
                cmd, cmd_uuid, ip_tag, extend_pattern=r"\s*__(save|render)__($|\s)"
            )
    ...
    def forever_run(self):
        t1 = Thread(
            ...
            args=(None, config.host_check_success_time * 3, False),
        )
