...
from .playbook_parser import simple_parser, get_line_by_num
...
class ClusterExecution(object):
    ...
    def __init__(self, cluster_id, redis_config_list, redis_connect=None):
        ...
        self.exe_uuid_list = (
            []
        )  # 正在运行的命令的uuid列表 如果依赖前项则逐个uuid检测获取结果再判断是否执行下一步
        self.pause_tag = 0  # 是否进行暂停
        self.stop_str = ""  # 用于标记执行结束的信息
    ...
    def redis_refresh(self):
        ...
        self.redis_client_list = [
            self.redis_send_client,
            self.redis_log_client,
            self.redis_tmp_client,
            self.redis_job_client,
            self.redis_config_client,
        ]
    ...
    def get_value(self, target_name, name_str):
        ...
        pattern = r"^(%s|%s|%s)\s*\..*" % (
            config.prefix_global,
            config.prefix_session,
            config.prefix_select,
        )
    ...
    def render(self, target, cmd):
        ...
        for c_ in re.findall(r"(?<={{)[^{}]*?{{[^{}]+?}}[^{}]*?(?=}})", cmd):
            ...
            if not v:
                # 双重渲染第一次的值不因该为空
                raise Exception(
                    "variable [%s] render error, double render value should not empty"
                    % c
                )
            # 直接进行替换
        ...
        for c in re.findall(r"(?<={{).+?(?=}})", cmd):
            c_r = re.sub(
                r"\s*\.\s*", sove_inner_keep, c
            ).strip()  # 去除字符串的左右空格 以及.左右的空格
    ...
    def exe(self, target, playbook, begin_line=0):
        ...
        stop_info["target"] = target

        self.redis_log_client.hmset(self.sum_key, stop_info)

        try:
            current_line = 0
            for playbook_cmd in simple_parser(playbook):
                ...
                if cmd:
                    self.redis_log_client.hset(self.current_uuid, "playbook_cmd", cmd)
                    is_continue = self.exe_single_line(cmd)
                    logger.debug(
                        "playbook line result ----------------[%s] %s"
                        % (cmd, is_continue)
                    )
                    if not is_continue:
                        break
            ...
            if not self.stop_str:
                if last_cmd_info:
                    if (
                        str(last_cmd_info["exit_code"]) == "0"
                        and not last_cmd_info["stderr"]
                    ):
                        self.stop_str = "done"
                    else:
                        self.stop_str = str(last_cmd_info["exit_code"])
                else:
                    self.stop_str = "done"
        except:
            logger_err.error(format_exc())
            self.stop_str = "unexpected err"

        stop_info["stop_str"] = self.stop_str
    ...
    def pause(self, key, pause_tag):
        ...
        if pause_tag:
            ...
            block_tag = self.redis_send_client.blpop(
                key, timeout=config.tmp_config_expire_sec
            )
    ...
    def exe_single_line(self, cmd):
        """执行一行playbook"""
        if re.match("^#", cmd) or re.match("^$", cmd):
            # 空白以及注释行单独判断
            self.stop_str = self.pause(self.block_key, self.pause_tag)

            if self.stop_str:
                self.redis_log_client.hset(
                    self.current_uuid, "exit_code", self.stop_str
                )
                self.redis_log_client.hset(self.current_uuid, "stderr", self.stop_str)
                self.redis_log_client.hset(self.current_uuid, "stdout", "")
                self.exe_next = False
                self.redis_log_client.expire(
                    self.current_uuid, config.cmd_log_expire_sec
                )
                return False
            ...
            # 跳过注释以及空白行
            logger.debug(
                "origin command: %s ---- <%s %s> will not execute"
                % (cmd, self.target, self.cluster_id)
            )

        else:
            self.redis_log_client.hset(
                self.current_uuid, "start_timestamp", time.time()
            )
            self.redis_log_client.hset(self.current_uuid, "origin_cmd", cmd)

            logger.debug(
                "origin command: %s ------------ <%s %s> %s"
                % (cmd, self.target, self.cluster_id, self.current_uuid)
            )
            try:
                cmd = self.render(self.target, cmd)
            except:
                logger_err.error(format_exc())
                self.redis_log_client.hset(
                    self.current_uuid, "exit_code", "render error"
                )
            ...
            # 没有预先切换主机则终止
                ...
                self.redis_log_client.hset(
                    self.current_uuid, "exit_code", "current_host null"
                )
                self.redis_log_client.hset(
                    self.current_uuid,
                    "stderr",
                    "should execute [<ip>] before any command",
                )
                self.redis_log_client.hset(self.current_uuid, "stdout", "")
                self.exe_next = False
                self.redis_log_client.expire(
            ...
            # 文件传输
                ...
                try:
                    ...
                    logger_err.error(format_exc())
                    self.redis_log_client.hset(
                        self.current_uuid, "exit_code", "__sync__ error"
                    )
                    # self.redis_log_client.hset(self.current_uuid,"stderr",format_exc())
            ...
            elif re.match(r"^__call__ ", cmd):
                try:
                    self.__call_exe(cmd)
                except Exception as e:
                    logger_err.error(format_exc())
                    self.redis_log_client.hset(
                        self.current_uuid, "exit_code", "__call__ error"
                    )
                    # self.redis_log_client.hset(self.current_uuid,"stderr",format_exc())
                    self.redis_log_client.hset(self.current_uuid, "stderr", str(e))
                    self.redis_log_client.hset(self.current_uuid, "stdout", "")
    ...
    def __single_exe(self, host, cmd, c_uuid=""):
        ...
        logger.debug(
            "-------------- <%s %s> %s %s %s accept"
            % (self.target, self.cluster_id, host, cmd, c_uuid)
        )
        ...
        if re.match("^wait$", cmd.strip()):
            ...
            if self.exe_next == False:
                self.redis_log_client.hset(
                    self.current_uuid, "exit_code", config.wait_exit_code
                )
                self.redis_log_client.hset(
                    self.current_uuid, "stderr", "wait command result check failed"
                )
            else:
            ...
            cmd = re.sub(r"&\s*?$", "", cmd)
            self.redis_send_client.rpush(
                config.prefix_cmd + exe_tag, cmd + config.spliter + c_uuid
            )
            r = ""
        else:
            self.redis_send_client.rpush(
    ...
    def __host_conn(self, current_host, current_uuid):
        if config.cluster_connect_control:
            if current_host not in self.connect_host_info:
                ...
                self.redis_send_client.rpush(
                    config.key_conn_control,
                    current_host + config.spliter + current_uuid,
                )
        else:
            # 将主机ip放入初始化队列 由其他线程后台初始化连接
            self.redis_send_client.rpush(
                config.key_conn_control, current_host + config.spliter + current_uuid
            )
    ...
    def __host_change(self, cmd, current_uuid):
        ...
        if self.current_host:
            ...
            def get_heart_beat():
                heart_beat = (
                    self.redis_send_client.get(config.prefix_heart_beat + current_uuid)
                    or 0
                )
                # 用于兼容本地连接
                heart_beat1 = (
                    self.redis_send_client.get(
                        config.prefix_heart_beat + self.current_host
                    )
                    or 0
                )
            ...
            # 存在心跳即可 不必判断时间？
            while (
                not float(get_heart_beat())
                and retry_flag < config.host_check_wait
                and self.exe_next
                and (not self.redis_log_client.hget(current_uuid, "exit_code"))
            ):
                time.sleep(config.host_check_time)
                retry_flag = retry_flag + 1
                self.redis_log_client.hset(
                    current_uuid, "desc", "check:" + str(retry_flag)
                )
    ...
    def __var_exe(self, cmd, current_uuid, var_prefix):
        ...
        if (
            os_type == "Linux"
            and (re.match(r".*`.+`.*", g_value) or re.match(r".*\$\(.+\).*", g_value))
        ) or (os_type == "Windows" and re.match(r".*%.+%.*", g_value)):
    ...
    def __select_var(self, cmd, current_uuid):
        ...
        # 使用临时变量存放
        self.redis_send_client.set(
            key_select_all, g_value, config.tmp_config_expire_sec
        )
        ...
        block_tag = self.redis_send_client.blpop(
            key_select, config.tmp_config_expire_sec
        )
        self.redis_send_client.delete(key_select_all)

        if block_tag:
            s_value = str(block_tag[-1])
            self.redis_tmp_client.hset(
                config.prefix_select + config.spliter + self.cluster_id,
                g_field,
                s_value,
            )
    ...
    def __check_result(
        self, c_uuid_list, check_result_block=True, ignore_last_err=False
    ):
        """
        阻塞获取命令的执行结果
        ...
        for c_uuid in c_uuid_list:
            ...
            if check_result_block:
                ...
                while continue_check:
                    if self.redis_send_client.get(self.kill_key):
                        # 可能存在kill操作
                        self.redis_send_client.expire(
                            self.kill_key, config.kill_cluster_expire_sec
                        )
                        self.exe_next = False
                        ...
                        r["exit_code"] = "kill"
                        logger.info(
                            "get kill signal in <%s %s> %s"
                            % (self.target, self.cluster_id, c_uuid)
                        )
                    else:
                ...
                ignore_last_err = (
                    True  # 不检测结果 则恒忽略此步的执行结果以继续下一步操作
                )
                r["stdout"] = ""
                r["stderr"] = ""
                r["exit_code"] = "0"
        ...
        logger.debug(
            "check result %s <%s %s>" % (str(r_list), self.target, self.cluster_id)
        )
        return r_list
    ...
    def __sync_exe(self, cmd_line):
        ...
        if len(_cmd) < 3:
            ...
            is_compress = (
                int(cmd_options["compress"]) == 1
                if "compress" in cmd_options
                else is_compress
            )
            ...
            is_partial = (
                ...
                if "partial" in cmd_options
                else is_partial
            )
            is_check = (
                int(cmd_options["check"]) == 1 if "check" in cmd_options else is_check
            )
            is_progress = (
                int(cmd_options["progress"]) == 1
                if "progress" in cmd_options
                else is_progress
            )
            batch_size = (
                int(cmd_options["batch"]) if "batch" in cmd_options else batch_size
            )
        ...
        logger.debug(
            "from_host [%s] to_host [%s] current_host [%s] remote_host [%s]"
            % (from_host, to_host, self.current_host, remote_host)
        )

        if (len(to_info) != 2 and len(from_info) != 2) or (
            len(to_info) == 2 and len(from_info) == 2
        ):
        ...
        from_passwd = password.decrypt(
            safe_decode(
                self.redis_config_client.hget(
                    config.prefix_realhost + from_host, "passwd"
                )
            )
        )
        ...
        to_passwd = password.decrypt(
            safe_decode(
                self.redis_config_client.hget(
                    config.prefix_realhost + to_host, "passwd"
                )
            )
        )
        to_user = safe_decode(
        ...
        # remote_host 可能还没有连接
        self.__host_change(
            "[%s]" % remote_host, self.current_uuid
        )  # 需要检查连接，否则可能出现命令队列先存在
        self.redis_log_client.hdel(
            self.current_uuid, "exit_code"
        )  # 需要移除主机连接的返回码，否则因为存在返回码导致会执行下一行命令
        ...
        if not to_path_info:
            ...
            if r["stderr"] or str(r["exit_code"]) != "0":
                raise Exception(
                    "__sync__ to_dir not exists, and `%s` stderr:%s, exit code:%s"
                    % (tmp_cmd, r["stderr"], r["exit_code"])
                )
        ...
        def __scp__(exe_host, from_host, real_try):
            ...
            real_try = real_try + 1
            self.redis_log_client.hset(
                self.current_uuid, "sync_desc", "count:%s" % real_try
            )
            logger.debug(
                "from_host [%s] to_host [%s] current_host [%s] remote_host [%s] exe_host [%s]"
                % (from_host, to_host, self.current_host, remote_host, exe_host)
            )
            if exe_host == from_host:
                # 执行命令的主机为文件存放的主机
                cmd_scp = (
                    """rsync %s --rsh='sshpass -p "%s" ssh -oStrictHostKeyChecking=no -p %s' %s %s@%s:%s"""
                    % (
                        rsync_option,
                        to_passwd,
                        to_ssh_port,
            ...
            logger.debug(
                "transfer command: [%s] to [%s] ------------ <%s %s> %s"
                % (cmd_line, cmd_scp, self.target, self.cluster_id, self.current_uuid)
            )
        ...
        def __scp_proxy(real_try):
            ...
            logger.debug(
                "run command: [%s] ------------ <%s %s> %s"
                % (cmd_localhost, self.target, self.cluster_id, self.current_uuid)
            )
            if proxy_host:
                exe_host = proxy_host
            ...
            scp_result = self.__check_result([self.current_uuid])[0]
            self.current_host = (
                current_host  # __check_result 会修改 self.current_host，因此要重置
            )
        ...
        def __scp_inner(real_try):
            ...
            if not is_from_file:
                ...
                r["exit_code"] = "1"
                self.redis_log_client.hset(
                    self.current_uuid, "exit_code", r["exit_code"]
                )
            ...
            cmd_paramiko = "__paramiko_sftp__ %s@%s:%s::%s %s@%s:%s::%s" % (
                from_user,
                from_ip,
                from_ssh_port,
                from_file,
            ...
            logger.debug(
                "run command: [%s] ------------ <%s %s> %s"
                % (cmd_paramiko, self.target, self.cluster_id, self.current_uuid)
            )
            ...
            # batch_size = 512*1024
            show_process_gap = int(
                transfer_size / (batch_size * 20)
            )  # 记录20次已传输大小
            ...
            with to_sftp_client.file(to_file, write_mode) as dest_file:
                ...
                self.redis_log_client.hset(
                    self.current_uuid,
                    "current_size",
                    begin_size + (i - 1) * batch_size + len(last_data),
                )
        ...
        for s in try_seq:
            if int(s) in range(1, 1 + len(scp_functions)):
                ...
                raise Exception(
                    "number in arg [try] should in %s"
                    % list(range(1, 1 + len(scp_functions)))
                )
        ...
        for s in _try_seq:
            ...
            self.redis_log_client.hdel(self.current_uuid, "exit_code")
            self.redis_log_client.hset(
                self.current_uuid, "stderr", ""
            )  # 将之前可能存在的stderr置为空，以免出现当前执行正确但存在之前的stderr
            scp_result, cmd_real, real_try = scp_function(real_try)

            if scp_result["exit_code"] in [0, "0"]:
                logger.debug(
                    "run command: [%s] success ------------ <%s %s> %s"
                    % (cmd_real, self.target, self.cluster_id, self.current_uuid)
                )
                ...
                if is_check:
                    if is_from_file:
                        ...
                        self.redis_log_client.hset(
                            self.current_uuid, "from_md5", from_md5
                        )
                        self.redis_log_client.hset(self.current_uuid, "to_md5", to_md5)
                        self.redis_log_client.hset(
                            self.current_uuid, "is_success", is_success
                        )
                        if not is_success:
                            self.redis_log_client.hset(
                                self.current_uuid, "exit_code", "md5 check failed"
                            )
                        ...
                        if is_success:
                            self.redis_log_client.hset(
                                self.current_uuid, "exit_code", "size check failed"
                            )
        ...
        self.redis_log_client.hset(
            self.current_uuid, "cmd_type", "EXTEND"
        )  # 可能因为执行shell命令导致类型被改变，因此再设置一次
        self.current_host = current_host

    def __call_exe(self, cmd):
        # __call__ 10 -constrict=^global.*$
        _cmd = cmd_split(cmd, 2)
        if len(_cmd) < 2:
            raise Exception("__call__ command length must not less than 2")

        call_line = _cmd[1]
        try:
            call_line = int(call_line)
        except ValueError:
            raise Exception("__call__ [%s] is not a int value" % call_line)

        call_constrict = ""
        if len(_cmd) > 2:
            cmd_options = cmd_options_split(_cmd[2])
            call_constrict = (
                cmd_options["constrict"]
                if "constrict" in cmd_options
                else call_constrict
            )

        # 从执行日志中获取已经之前已经转换好的命令，如果再次运行会出现获取为空值，不采用这种实现方法
        # 从playbook中获取指定行的命令
        logger.debug("__call__ plabook ---- %s" % self.playbook)
        cmd_call = get_line_by_num(self.playbook, call_line).strip()

        logger.debug("__call__ ---- origin [%s] ----- real [%s]" % (cmd, cmd_call))

        if call_constrict:
            if not re.match(call_constrict, cmd_call):
                raise Exception(
                    "[__call__ -constrict=%s] regexp check failed for [%s]"
                    % (call_constrict, cmd_call)
                )

        # self.__single_exe(self.current_host,cmd_call,self.current_uuid) # 不用这种方法，否则大部分扩展命令执行不了
        self.exe_single_line(cmd_call)
