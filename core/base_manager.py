...
class BaseManager(object):
    ...
    def localhost(self):
        ...
        lh = LocalHost(
            [self.redis_send_config, self.redis_log_config],
            self.listen_tag,
    ...
    def is_host_alive(self, ip, lock=True):
        ...
        if lock:
            check_flag = self.redis_send_client.set(
                config.prefix_check_flag + ip, 1, nx=True
            )
        else:
            check_flag = not self.redis_send_client.get(config.prefix_check_flag + ip)

        # 拿不到判断的flag则默认服务是连接的 存在心跳则服务是连接的
        if not check_flag or self.redis_send_client.exists(
            config.prefix_heart_beat + ip
        ):
            is_alive = True
        else:
            is_alive = False
    ...
    def conn_host(self, init_host, redis_connect, init_host_uuid):
        ...
        try:
            ...
            if not self.is_host_alive(init_host):
                host_info = self.redis_config_client.hgetall(
                    config.prefix_realhost + init_host
                )
                if config.cluster_connect_control:
                ...
                if not ("ip" in host_info):
                    self.redis_log_client.hset(
                        init_host_uuid, "exit_code", "host info err"
                    )
                    logger_err.error(
                        "< %s > init failed, host info error, ip not exist" % init_host
                    )

                elif (
                    not self.redis_send_client.llen(config.prefix_cmd + init_host)
                    or not config.host_start_without_cmd
                ):
                    ...
                    try:
                        self.redis_send_client.set(
                            config.prefix_initing + init_host, time.time()
                        )
                        h = RemoteHost(
                            host_info, [self.redis_send_config, self.redis_log_config]
                        )
                        h.forever_run()
                        logger.info("< %s > init success" % init_host)
                    except:
                        self.redis_log_client.hset(
                            init_host_uuid, "exit_code", "init failed"
                        )
                        self.redis_log_client.hset(
                            init_host_uuid, "stderr", format_exc()
                        )
                        self.redis_send_client.set(
                            config.prefix_initing + init_host, -1
                        )
                        logger_err.error(format_exc())
                        logger_err.error("< %s > init failed" % init_host)

                    self.redis_send_client.expire(
                        config.prefix_initing + init_host,
                        config.initing_host_flag_expire_sec,
                    )
                else:
                    self.redis_log_client.hset(init_host_uuid, "exit_code", "cmd exist")
                    logger_err.error(
                        "< %s > not shutdown clean before,will not init. please check cmd_%s"
                        % (init_host, init_host)
                    )

                self.redis_send_client.delete(
                    config.prefix_check_flag + init_host
                )  # 连接不存在时操作完毕才释放锁
            else:
                self.redis_log_client.hset(
                    init_host_uuid, "stdout", "connect exist,init skip"
                )
