...
class JobManager(BaseManager):
        ...
        while True:
            ...
            if close_tag:
                ...
                if "proxy" in host_info and host_info["proxy"].strip():
                    proxy_list_name = (
                        config.proxy_tag + ":" + host_info["proxy"].strip()
                    )
                    self.redis_send_client.rpush(proxy_list_name, _init_host)
                    logger.debug(
                        "< %s > will init connect by proxy, not here" % init_host
                    )
        ...
        while True:
            ...
            for k in self.redis_send_client.scan_iter(hb_tag + "*"):
                ...
                if self.redis_send_client.exists(config.prefix_initing + ip):
                    ...
                    close_flag = False
                    log_host_len = self.redis_log_client.llen(
                        config.prefix_log_host + ip
                    )
                    if log_host_len > 0:
                        log_host_last = self.redis_log_client.lrange(
                            config.prefix_log_host + ip,
                            log_host_len - 1,
                            log_host_len - 1,
                        )[0]
                        log_host_last_dict = self.redis_log_client.hgetall(
                            log_host_last
                        )
                        log_host_last_end = "end_timestamp" in log_host_last_dict
                        if log_host_last_end:
                            if (
                                time.time() - float(log_host_last_dict["end_timestamp"])
                            ) > config.host_close_time:
                                # 最后的执行时间距离现在超过指定时间，可关闭
                                close_flag = True
                                logger.debug(
                                    "< %s > has no execution for a long time,will close.last exection info: %s"
                    ...
                    if close_flag:
                        self.redis_send_client.rpush(
                            config.key_conn_control, config.pre_close + ip
                        )
                    else:
    ...
    def __real_job_exe(self, job_id):
        ...
        for oc in job["target"].split(","):
            ...
            if self.redis_tmp_client.hgetall(new_c):
                ...
                if not self.redis_config_client.hgetall(c):
                    self.redis_log_client.hmset(
                        config.prefix_sum + cluster_id, {"stop_str": "not exist"}
                    )
                    continue
            ...
            try:
                ...
                self.redis_tmp_client.expire(new_c, config.tmp_config_expire_sec)
                self.redis_log_client.hmset(
                    config.prefix_sum + cluster_id, {"stop_str": "runing failed"}
                )
        ...
        self.redis_log_client.hmset(
            config.prefix_log_job + job_id.split(config.prefix_job)[1], log_job
        )
    ...
    def __job_exe(self):
        ...
        while True:
            job_id = None
            try:
                job_id = self.redis_send_client.blpop(
                    config.key_job_list
                )  # redis重新连接运行会有连接报错
                job_id = job_id[1]
            except:
