...
class AbstractHost(AbstractHostBase):
    ...
    def single_run(
        self, cmd, cmd_uuid, ip_tag, extend_pattern=r"\s*__\w+__($|\s)", real_ip=None
    ):
        """
        单个命令的执行
        ...
        try:
            ...
            if (stdout, stderr, exit_code) == (None, None, None):
                ...
                if extend_pattern and re.match(extend_pattern, cmd):
                    exe_result, stdout, stderr, exit_code = self.__exe_extend(
                        cmd, exe_result, ip_tag
                    )
                    ...
                    stdout, stderr, exit_code = self.exe_cmd(
                        cmd,
                        background_log_set=gen_background_log_set(
                            cmd_uuid, self.redis_log_client
                        ),
                        ip_tag=ip_tag,
                    )
        except:
            ...
            stdout, stderr, exit_code = (
                "",
                "some error happen when execute,please check the log",
                1,
            )
    ...
    def __exe_extend(self, cmd_line, exe_result, ip_tag):
        ...
        if cmd == "__put__":
            ...
            local_md5, remote_md5, is_success, msg = self.send_file(
                local_file,
                remote_path,
                self.gen_set_info(cmd_uuid),
                self.gen_set_step(cmd_uuid),
                ip_tag=ip_tag,
            )
            ...
            local_md5, remote_md5, is_success, msg = self.get_file(
                local_path,
                remote_file,
                self.gen_set_info(cmd_uuid),
                self.gen_set_step(cmd_uuid),
            ...
            if len(_cmd) < 3:
                ...
                for k in list(cmd_options.keys()):
                    ...
                    if match:
                        ...
                        cmd_options[real_key] = safe_decode(
                            self.read_file(filepath, "r")
                        )
            ...
            # 扩展目录中的扩展命令
            filename, __cmd = is_file_in_dir(
                cmd,
                self.extends_postfixs,
                os.path.join(config.base_dir, self.extends_dir),
            )
    ...
    def send_file(self, local_file, remote_path, set_info, set_step, ip_tag):
        ...
        # 可以在等待过程中删除标记结束等待
        while wait_flag and self.redis_log_client.hexists(
            config.prefix_put + ip_tag, local_md5
        ):
            exist_remote_file = self.redis_log_client.hget(
                config.prefix_put + ip_tag, local_md5
            )
            if exist_remote_file:
                ...
                try:
                    ...
                    # 校验MD5并复制文件
                    remote_md5, local_md5, is_success, error_msg = self.copy_file(
                        exist_remote_file,
                        remote_file,
                        local_md5,
                        local_filesize,
                        config.is_copy_by_link,
                        set_info,
                        set_step,
                        ip_tag,
                    )
                    ...
                    if is_success:
                        ...
                        logger.debug(
                            "copy but faild:  %s %s %s %s"
                            % (remote_md5, local_md5, is_success, error_msg)
                        )
        ...
        if put_flag:
            ...
            try:
                local_md5, remote_md5, is_success, error_msg = self.put_file(
                    local_md5, local_file, remote_path, set_info, set_step, ip_tag
                )

                # redis断开即导致上传失败 无需确保日志回写
                if is_success:
                    self.redis_log_client.hset(
                        config.prefix_put + ip_tag, local_md5, remote_file
                    )
                else:
    ...
    def set_log(self, ip_tag, exe_result, is_update=True):
        ...
        if is_update:
            self.redis_send_client.hdel(
                config.prefix_log_now + ip_tag, log_uuid
            )  # 执行结束，删除正在执行的命令信息
            self.redis_log_client.expire(
                log_uuid, config.cmd_log_expire_sec
            )  # 获取返回值后设置日志过期时间
        else:
            self.redis_send_client.hset(
                config.prefix_log_now + ip_tag, log_uuid, time.time()
            )  # 设置正在执行的命令信息
    ...
    def heart_beat(
        self,
        log_out=None,
        expire_time=config.host_check_success_time,
        session_check=True,
    ):
        """
        心跳
            ...
            if session_check:
                if config.heart_beat_type == 1:
                    try:
                        ...
                        try:
                            self.redis_send_client.publish(
                                config.key_kill_host, self.ip_tag
                            )
                        except:
                            pass
            ...
            try:
                for ip_tag in self.parallel_list:
                    ...
                    self.redis_send_client.set(
                        config.prefix_heart_beat + ip_tag, time.time(), expire_time
                    )
    ...
    def close_conn(self, wait=False):
        ...
        try:
            self.redis_send_client.set(
                self.cloing_key, time.time()
            )  # 标记host处于关闭状态，不再执行新命令
        except:
            pass
        ...
        self.redis_send_client.expire(
            self.cloing_key, config.closing_host_flag_expire_sec
        )
        self.redis_send_client.delete(
            self.cmd_key
        )  # 清空未执行的队列防止启动时错误地运行
