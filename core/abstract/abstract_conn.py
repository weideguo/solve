        ...
        try:
            ...
            if not self.host_queue_info[ip_tag].full():
                ...
                self.redis_send_client.rpush(
                    config.prefix_cmd + ip_tag, cmd + config.spliter + cmd_uuid
                )
                time.sleep(0.1)
        except:
            ...
            if list(self.redis_send_client.scan_iter(config.prefix_cmd + "*")):
                for k in self.redis_send_client.scan_iter(config.prefix_cmd + "*"):
                    ...
                    if allcmd:
                        ...
                        cmd, cmd_uuid = command_split(
                            allcmd, config.spliter, config.uuid_len
                        )

                        t = Thread(
                            target=self.__single_exe, args=(cmd, cmd_uuid, ip_tag)
                        )
                        t.start()
                    else:
                        self.thread_q.get(block=False)
