...
if __name__ == "__main__":
    ...
    rc = RedisConn(
        max_connections=config.shared_redis_pool_size
    )  # max_connections只是对于单个连接池 使用时每个db对应一个连接池
    ...
    if is_start_fileserver:
        print(
            "fileserver listen on  \033[1;32m %s:%d \033[0m"
    ...
    def init_set():
        ...
        if config.clear_start and Manager != ProxyManager:
            clear_keys = [
                ...
                config.prefix_cmd,
                config.prefix_heart_beat,
                config.prefix_log_now,
            ]
