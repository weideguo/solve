...
class SolveLock(object):
    ...
    def get_cmd_lock(self):
        ...
        if not self.redis_send_client.exists(self.heart_beat_key):
            return self.redis_send_client.lrange(
                self.cmd_key, 0, self.redis_send_client.llen(self.cmd_key)
            )
        else:
            return []
    ...
    def release_lock(self, lock_str, skip_err=False, spliter=","):
        ...
        for _lock in lock_str.split(spliter):
            if _lock in self.lock_list:
                ...
                print(
                    "%s not found in lock list %s" % (_lock, ",".join(self.lock_list))
                )
                break
...
if __name__ == "__main__":
    ...
    if (
        len(sys.argv) == 1
        or len(sys.argv) > 3
        or (len(sys.argv) == 2 and sys.argv[1] in ["--help", "-h", "-a", "-r", "-f"])
        or (len(sys.argv) == 3 and sys.argv[1] not in ["-a", "-r", "-f"])
    ):
        print("useage:")
