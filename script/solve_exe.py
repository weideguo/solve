...
class SolveExe(object):
    ...
    job_info["begin_line"] = (
        0  # 从第几行开始执行 之前的命令全部跳过 非零时必须设置begin_host
    )
        ...
        with open(playbook, "r") as f:
            ...
            while l:
                ...
                if not re.match("^#", l):
                    ...
                    session_vars = session_vars + re.findall(
                        "(?<={{" + config.prefix_session + "\.).*?(?=}})", l
                    )
                l = f.readline()
    ...
    def check_job_result(self, kill=False):
        ...
        if log_job_dict:
            ...
            for lc in log_target:
                ...
                cluster_log_last_info = self.redis_log_client.hgetall(
                    "sum_" + cluster_id
                )  # 获取最后一个命令的日志
                # print(cluster_log_last_info)
                if ("stop_str" in cluster_log_last_info) and (
                    "end_timestamp" in cluster_log_last_info
                ):
                    ...
                    if kill:
                        ...
                        self.redis_send_client.set(
                            config.prefix_kill + cluster_id, time.time()
                        )
                        self.redis_log_client.hset(
                            config.prefix_sum + cluster_id, "stop_str", "killing"
                        )
    ...
    def is_job_done(self, job_result):
        if job_result["uncomplete"] or (
            not job_result["complete"]
            and not job_result["uncomplete"]
            and not job_result["fail"]
        ):
            return False
        else:
            return True
...
if __name__ == "__main__":
    ...
    if (len(sys.argv) == 2 and sys.argv[1] not in ["--block", "-b"]) or (
        len(sys.argv) > 2
    ):
        print("useage:")
