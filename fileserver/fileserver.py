...
def file_upload():
    ...
    if os.path.isfile(save_path):
        # msg="路径为文件"
        return jsonify(
            {"status": -1, "file": save_path, "msg": "arg path should not be a file"}
        )
...
def file_manage(args):
    ...
    if args == "download":
        ...
        if os.path.isfile(filename):
            ...
            r = jsonify(
                {"status": -1, "file": filename, "msg": "arg file is not a file"}
            )
    ...
    if args == "list":
        ...
        if not os.path.isfile(root_path) and os.path.exists(root_path):
            ...
            dirs.sort()
            return jsonify(
                {"status": 1, "path": root_path, "files": files, "dirs": dirs}
            )
        else:
            # return jsonify({"status":-1,"path":root_path,"msg":"路径不为目录"})
            return jsonify(
                {"status": -1, "path": root_path, "msg": "arg path is not a directory"}
            )
    ...
    dictConfig(
        {
            ...
            "root": {"level": "INFO", "handlers": ["fileoutput"]},
        }
    )
