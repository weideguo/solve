...
class MySSH(object):
        ...
        **kwargs
        ...
        if is_copy_by_link:
            try:
                ...
                ftp_client.putfo(
                    ftp_client.open(exist_remote_file),
                    remote_file,
                    local_filesize,
                    callback=set_info,
                )
    ...
    def get_file(
        self, local_path, remote_file, set_info=None, set_step=None, *arg, **kwargs
    ):
        """
        下载文件到本地 文件名不变
    ...
    def is_remote_file(
        self, ftp_client, remote_file, local_md5="", set_step=None, *arg, **kwargs
    ):
        """
        判断远端文件是否存在以及md5是否一致
        """
        ...
        try:
            ...
            try:
                if remote_filename in ftp_client.listdir(remote_dir):
                    ...
                    try:
                        ftp_client.posix_rename(
                            remote_file, remote_file + "_" + str(time.time())
                        )
                    except:
                        ftp_client.rename(
                            remote_file, remote_file + "_" + str(time.time())
